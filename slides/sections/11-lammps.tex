...
./lammps/build/lmp -h | head
    ...
    \begin{successbox}[You Built 65 Packages]
      \footnotesize \texttt{lmp -h} shows GPU (mixed, sm\_80) + KOKKOS (CUDA, AMPERE80) + 24 cores
    \end{successbox}
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Input Script --- The Recipe}
  \begin{columns}[T,onlytextwidth]
    \column{0.5\textwidth}
    \begin{minted}[fontsize=\footnotesize,bgcolor=codebg]{text}
units real
atom_style full
...
\begin{frame}{Execution --- Log, Trajectory, Analysis}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{tikzpicture}[scale=1.0]
      \node[draw=accentcyan,fill=darkgray,rounded corners=1mm,minimum width=2.2cm,minimum height=0.6cm] (a) at (0,0) {\color{fgwhite}\footnotesize in.lj};
      \node[draw=neongreen,fill=darkgray,rounded corners=1mm,minimum width=1.1cm,minimum height=0.6cm] (b) at (2.4,0.6) {\color{neongreen}\footnotesize log};
      \node[draw=neonyellow,fill=darkgray,rounded corners=1mm,minimum width=1.4cm,minimum height=0.6cm] (c) at (2.4,-0.6) {\color{neonyellow}\footnotesize dump.*};
