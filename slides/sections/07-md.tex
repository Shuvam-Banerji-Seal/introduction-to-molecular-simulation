...
\begin{frame}{Newton's Equations --- The Engine}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{tikzpicture}[scale=0.9]
      \node[draw=accentcyan,fill=darkgray,rounded corners=1mm,minimum width=2.2cm,minimum height=0.7cm] (u) at (0,1) {\color{fgwhite}\tiny $U(\mathbf{R})$};
      \node[draw=neongreen,fill=darkgray,rounded corners=1mm,minimum width=2.2cm,minimum height=0.7cm] (f) at (0,0) {\color{neongreen}\tiny $\mathbf{F}=-\nabla U$};
      \node[draw=neonyellow,fill=darkgray,rounded corners=1mm,minimum width=2.2cm,minimum height=0.7cm] (r) at (0,-1) {\color{neonyellow}\tiny $\mathbf{r}(t)$};
      \draw[->,thick,accentcyan] (u) -- (f);
      \draw[->,thick,neongreen] (f) -- (r);
      \draw[->,thick,neonyellow,dashed] (r) to[bend left=30] (u) node[midway,right]{update $U$};
    \end{tikzpicture}
  \end{columns}
\end{frame}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=0.6]
      \foreach \idx/\txt in {0/{$\mathbf{r}_0,\mathbf{v}_0$},1/Forces,2/Integrate,3/Trajectory} {
        \node[draw=accentcyan,rounded corners=2mm,fill=darkgray,minimum width=2.1cm,minimum height=0.7cm,align=center] (n\idx) at (\idx*2.7,0) {\color{fgwhite}\tiny \txt};
      }
      \draw[->,thick,accentcyan] (n0) -- (n1);
      \draw[->,thick,accentcyan] (n1) -- (n2);
      \draw[->,thick,accentcyan] (n2) -- (n3);
      \node[draw=neonyellow,fill=neonyellow!15,rounded corners=1mm,inner sep=1mm] at (1.35,1) {IC: Maxwell--Boltzmann};
      \node[draw=neongreen,fill=neongreen!15,rounded corners=1mm,inner sep=1mm] at (4.05,1) {Equil. \ensuremath{\rightarrow} Prod.};
    \end{tikzpicture}
  \end{center}
  \begin{pitemize}
...
\begin{frame}{Meme --- Pikachu Surprise}
  \begin{center}
    ...
    {\scriptsize\color{softgray} When dt=5 fs and system explodes --- \textit{surprised}}
  \end{center}
\end{frame}
