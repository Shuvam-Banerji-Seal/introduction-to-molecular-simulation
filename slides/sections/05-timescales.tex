...
\begin{frame}{The Timestep $dt$ --- Heartbeat of MD}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{tikzpicture}[scale=0.6]
      ...
      \only<2>{\node[draw=neonpink,fill=neonpink!15,rounded corners=1mm,inner sep=1mm] at (1.4,1) {SHAKE \ensuremath{\rightarrow} $dt=2$ fs!};}
    \end{tikzpicture}
  \end{columns}
\end{frame}
...
\begin{frame}{Hierarchy of Time}
  \begin{center}
    \begin{tikzpicture}[scale=0.9]
      \draw[accentcyan,thick] (0,0) -- (10,0);
      \foreach \x/\lab/\t in {0.5/Bond vibr./1 fs,2/ Angle/10 fs,3.5/Rotation/1 ps,5.5/Diffusion/10 ps,7.5/Folding/$\mu$s,9.5/Rare/ ms} {
        \draw[accentcyan,thick] (\x,0.1)--(\x,-0.1);
        \node[align=center] at (\x,0.5) {\lab\\\color{softgray}\tiny \t};
        \fill[neongreen] (\x,0) circle (2pt);
      }
      \draw[->,thick,neonyellow] (0,-0.8) -- (3,-0.8) node[midway,below]{MD resolves};
      \draw[->,thick,neonpink] (5,-0.8) -- (9.5,-0.8) node[midway,below]{need enhanced sampling};
    \end{tikzpicture}
  \end{center}
  \begin{pitemize}
...
\begin{frame}{Meme --- Timescales}
  \begin{center}
    \memeinclude[width=0.75\linewidth]{assets/memes/water_meme.png}
    {\tiny\color{softgray} fs vibrations live inside ps water, inside $\mu$s proteins --- Russian doll of time}
  \end{center}
\end{frame}
